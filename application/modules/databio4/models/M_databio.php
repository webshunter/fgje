...
class M_databio extends CI_Model{
 ...
 $result = DBS::conn("SELECT * FROM personalblk where nodaftar='$idbio'");
    ...
    function hitunganfingernodaftujuh($idbio){
        $kode_desa="";
        $result = DBS::conn("SELECT DATE_ADD(adm_tglreg, INTERVAL 68 DAY) as expired FROM personalblk WHERE nodaftar='$idbio'");
                 while($row = mysqli_fetch_array($result)){
                $kode_desa=$row['expired'];
            }
        return $kode_desa;
       ...
       function hitunganfingernodaft($idbio){
        $kode_desa="";
        $result = DBS::conn("SELECT DATE_ADD(adm_tglreg, INTERVAL 41 DAY) as expired FROM personalblk WHERE nodaftar='$idbio'");
                 while($row = mysqli_fetch_array($result)){
                $kode_desa=$row['expired'];
            }
        return $kode_desa;
