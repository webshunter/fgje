...
function tampilkan_data_operasi($key){
    $data = $this->db->query("SELECT * FROM skillcondition WHERE id_biodata = '$key'")->row();
...
function tampilkan_data_alergi($key){
    $data = $this->db->query("SELECT * FROM skillcondition WHERE id_biodata = '$key'")->row();
    if (isset($data->operasi)) {
        $kirim = $data->alergi;
    }else{
...
function tampilkan_data_rokok($key){
    $data = $this->db->query("SELECT * FROM skillcondition WHERE id_biodata = '$key'")->row();
    if (isset($data->operasi)) {
        $kirim = $data->merokok;
    }else{
