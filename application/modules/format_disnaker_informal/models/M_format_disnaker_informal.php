<?php
class M_format_disnaker_informal extends CI_Model{
    function __construct(){
        parent::__construct();
    }
 
  function tampil_data_personal(){
		$sql = "SELECT personal.alamat as alamatnya,personal.nama as namanya, personal.*,family.*,majikan.*,dataagen.*,paspor.*,format_disnaker_informal.* FROM format_disnaker_informal
				LEFT JOIN personal 
				ON format_disnaker_informal.id_biodata = personal.id_biodata
				LEFT JOIN family 
				ON format_disnaker_informal.id_biodata = family.id_biodata
				LEFT JOIN majikan 
				ON format_disnaker_informal.id_biodata = majikan.id_biodata
				INNER JOIN paspor 
				ON format_disnaker_informal.id_biodata = paspor.id_biodata
				LEFT JOIN dataagen 
				ON majikan.kode_agen = dataagen.id_agen
				WHERE format_disnaker_informal.id_biodata LIKE 'mi%'
				;";
	 ...
	 function tampil_data_tki(){
		$sql = "SELECT * FROM personal	WHERE personal.id_biodata LIKE 'mi%'
				;";
                $query = $this->db->query($sql);
	...
	function simpan_data_surat(){
		...
		$this->db->insert('format_disnaker_informal',$data);
	}
	...
	function edit_data_surat(){
		...
		$this->db->update('format_disnaker_informal',$data);
	}
	
	function hapus_data_surat() {
		$id_karep = $this->input->post('id_karep');
		$this->db->where('id_karep', $id_karep);
		$this->db->delete('format_disnaker_informal');
	}
