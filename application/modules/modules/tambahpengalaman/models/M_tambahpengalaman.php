...
function tampil_data_posisi(){
	...
	function tampil_data_pengalamandetail($idnya,$id_pengalaman){
		$sql = "SELECT * FROM pengalaman WHERE id_biodata='".$idnya."' AND id_pengalaman='".$id_pengalaman."'";
                $query = $this->db->query($sql);
	...
	function ubahpengalaman() {
		...
		if($this->input->post('rawatbinatang')=="1") {
			...
			$rawatbinatang="0";
 			...
 			$this->db->where('id_pengalaman', $this->input->post('idpengalaman'));
