<?php
class M_upload_arc extends CI_Model{
	...
	function simpan_data_arc() {
        ...
        $config['upload_path'] = './assets/upload_arc/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_arc', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_arc(){
		$sql = "SELECT * FROM upload_arc";
                $query = $this->db->query($sql);
	...
	function update_data_arc() {
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_arc', $id_arc);
		$this->db->update('upload_arc', $data);
	...
	function hapus_data_arc() {
		$id = $this->input->post('id_arc');
		$this->db->where('id_arc', $id);
		$this->db->delete('upload_arc');
	}
