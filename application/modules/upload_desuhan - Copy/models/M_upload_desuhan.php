...
class M_upload_desuhan extends CI_Model{
	...
	function simpan_data_desuhan() {
        ...
        {
            	 $data = array(
            	 	'id_suhan'=>$this->input->post('idbiodata'),
...
function tampil_data_personal($idnya){
	...
	function tampil_data_desuhan($idnya){
		$sql = "SELECT * FROM upload_desuhan WHERE id_suhan='".$idnya."'";
                $query = $this->db->query($sql);
	...
	function hapus_data_desuhan() {
		$id = $this->input->post('id_desuhan');
		$this->db->where('id_desuhan', $id);
		$this->db->delete('upload_desuhan');
	}
