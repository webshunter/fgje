<div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0 text-dark">Ubah personal</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{ url('toko') }}">Home</a></li>
            </ol>
        </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
    </div>
    <!-- /.content-header -->

    <section class="content">
      <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="<?= site_url('admin/personal/update') ?>" method="post" enctype="multipart/form-data">
                        
        <?=
            form::input([
                "type" => "hidden",
                "fc" => "id_personal",
                "value" => $form_data->id_personal,
            ])
        ?>
    
                <?=
                    form::input([
                        "title" => "id_biodata",
                        "type" => "text",
                        "fc" => "id_biodata",
                        "placeholder" => "tambahkan id_biodata",
                        "value" => $form_data->id_biodata,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "negara1",
                        "type" => "text",
                        "fc" => "negara1",
                        "placeholder" => "tambahkan negara1",
                        "value" => $form_data->negara1,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "negara2",
                        "type" => "text",
                        "fc" => "negara2",
                        "placeholder" => "tambahkan negara2",
                        "value" => $form_data->negara2,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "calling",
                        "type" => "text",
                        "fc" => "calling",
                        "placeholder" => "tambahkan calling",
                        "value" => $form_data->calling,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "skill1",
                        "type" => "text",
                        "fc" => "skill1",
                        "placeholder" => "tambahkan skill1",
                        "value" => $form_data->skill1,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "skill2",
                        "type" => "text",
                        "fc" => "skill2",
                        "placeholder" => "tambahkan skill2",
                        "value" => $form_data->skill2,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "skill3",
                        "type" => "text",
                        "fc" => "skill3",
                        "placeholder" => "tambahkan skill3",
                        "value" => $form_data->skill3,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "kode_proses",
                        "type" => "text",
                        "fc" => "kode_proses",
                        "placeholder" => "tambahkan kode_proses",
                        "value" => $form_data->kode_proses,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "kode_sponsor",
                        "type" => "text",
                        "fc" => "kode_sponsor",
                        "placeholder" => "tambahkan kode_sponsor",
                        "value" => $form_data->kode_sponsor,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "kode_agen",
                        "type" => "text",
                        "fc" => "kode_agen",
                        "placeholder" => "tambahkan kode_agen",
                        "value" => $form_data->kode_agen,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "nama",
                        "type" => "text",
                        "fc" => "nama",
                        "placeholder" => "tambahkan nama",
                        "value" => $form_data->nama,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "nama_mandarin",
                        "type" => "text",
                        "fc" => "nama_mandarin",
                        "placeholder" => "tambahkan nama_mandarin",
                        "value" => $form_data->nama_mandarin,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "jeniskelamin",
                        "type" => "text",
                        "fc" => "jeniskelamin",
                        "placeholder" => "tambahkan jeniskelamin",
                        "value" => $form_data->jeniskelamin,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "notelp",
                        "type" => "text",
                        "fc" => "notelp",
                        "placeholder" => "tambahkan notelp",
                        "value" => $form_data->notelp,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "notelpkel",
                        "type" => "text",
                        "fc" => "notelpkel",
                        "placeholder" => "tambahkan notelpkel",
                        "value" => $form_data->notelpkel,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "tanggaldaftar",
                        "type" => "text",
                        "fc" => "tanggaldaftar",
                        "placeholder" => "tambahkan tanggaldaftar",
                        "value" => $form_data->tanggaldaftar,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "tinggi",
                        "type" => "text",
                        "fc" => "tinggi",
                        "placeholder" => "tambahkan tinggi",
                        "value" => $form_data->tinggi,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "berat",
                        "type" => "text",
                        "fc" => "berat",
                        "placeholder" => "tambahkan berat",
                        "value" => $form_data->berat,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "hp",
                        "type" => "text",
                        "fc" => "hp",
                        "placeholder" => "tambahkan hp",
                        "value" => $form_data->hp,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "hpkel",
                        "type" => "text",
                        "fc" => "hpkel",
                        "placeholder" => "tambahkan hpkel",
                        "value" => $form_data->hpkel,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "warganegara",
                        "type" => "text",
                        "fc" => "warganegara",
                        "placeholder" => "tambahkan warganegara",
                        "value" => $form_data->warganegara,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "tempatlahir",
                        "type" => "text",
                        "fc" => "tempatlahir",
                        "placeholder" => "tambahkan tempatlahir",
                        "value" => $form_data->tempatlahir,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "tgllahir",
                        "type" => "text",
                        "fc" => "tgllahir",
                        "placeholder" => "tambahkan tgllahir",
                        "value" => $form_data->tgllahir,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "agama",
                        "type" => "text",
                        "fc" => "agama",
                        "placeholder" => "tambahkan agama",
                        "value" => $form_data->agama,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "status",
                        "type" => "text",
                        "fc" => "status",
                        "placeholder" => "tambahkan status",
                        "value" => $form_data->status,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "tglmenikah",
                        "type" => "text",
                        "fc" => "tglmenikah",
                        "placeholder" => "tambahkan tglmenikah",
                        "value" => $form_data->tglmenikah,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "pendidikan",
                        "type" => "text",
                        "fc" => "pendidikan",
                        "placeholder" => "tambahkan pendidikan",
                        "value" => $form_data->pendidikan,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "alamat",
                        "type" => "text",
                        "fc" => "alamat",
                        "placeholder" => "tambahkan alamat",
                        "value" => $form_data->alamat,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "alamatlengkap",
                        "type" => "text",
                        "fc" => "alamatlengkap",
                        "placeholder" => "tambahkan alamatlengkap",
                        "value" => $form_data->alamatlengkap,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "provinsi",
                        "type" => "text",
                        "fc" => "provinsi",
                        "placeholder" => "tambahkan provinsi",
                        "value" => $form_data->provinsi,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "mandarin",
                        "type" => "text",
                        "fc" => "mandarin",
                        "placeholder" => "tambahkan mandarin",
                        "value" => $form_data->mandarin,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "taiyu",
                        "type" => "text",
                        "fc" => "taiyu",
                        "placeholder" => "tambahkan taiyu",
                        "value" => $form_data->taiyu,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "inggris",
                        "type" => "text",
                        "fc" => "inggris",
                        "placeholder" => "tambahkan inggris",
                        "value" => $form_data->inggris,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "cantonese",
                        "type" => "text",
                        "fc" => "cantonese",
                        "placeholder" => "tambahkan cantonese",
                        "value" => $form_data->cantonese,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "hakka",
                        "type" => "text",
                        "fc" => "hakka",
                        "placeholder" => "tambahkan hakka",
                        "value" => $form_data->hakka,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "foto",
                        "type" => "text",
                        "fc" => "foto",
                        "placeholder" => "tambahkan foto",
                        "value" => $form_data->foto,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "statusaktif",
                        "type" => "text",
                        "fc" => "statusaktif",
                        "placeholder" => "tambahkan statusaktif",
                        "value" => $form_data->statusaktif,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "indukagen",
                        "type" => "text",
                        "fc" => "indukagen",
                        "placeholder" => "tambahkan indukagen",
                        "value" => $form_data->indukagen,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "kirimbio",
                        "type" => "text",
                        "fc" => "kirimbio",
                        "placeholder" => "tambahkan kirimbio",
                        "value" => $form_data->kirimbio,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "pk",
                        "type" => "text",
                        "fc" => "pk",
                        "placeholder" => "tambahkan pk",
                        "value" => $form_data->pk,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "pap",
                        "type" => "text",
                        "fc" => "pap",
                        "placeholder" => "tambahkan pap",
                        "value" => $form_data->pap,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "remark",
                        "type" => "text",
                        "fc" => "remark",
                        "placeholder" => "tambahkan remark",
                        "value" => $form_data->remark,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "datafoto",
                        "type" => "text",
                        "fc" => "datafoto",
                        "placeholder" => "tambahkan datafoto",
                        "value" => $form_data->datafoto,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "keterangan",
                        "type" => "text",
                        "fc" => "keterangan",
                        "placeholder" => "tambahkan keterangan",
                        "value" => $form_data->keterangan,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "keterangan2",
                        "type" => "text",
                        "fc" => "keterangan2",
                        "placeholder" => "tambahkan keterangan2",
                        "value" => $form_data->keterangan2,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "lokasikerja",
                        "type" => "text",
                        "fc" => "lokasikerja",
                        "placeholder" => "tambahkan lokasikerja",
                        "value" => $form_data->lokasikerja,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "idpemilik",
                        "type" => "text",
                        "fc" => "idpemilik",
                        "placeholder" => "tambahkan idpemilik",
                        "value" => $form_data->idpemilik,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "statterbang",
                        "type" => "text",
                        "fc" => "statterbang",
                        "placeholder" => "tambahkan statterbang",
                        "value" => $form_data->statterbang,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "ketdok",
                        "type" => "text",
                        "fc" => "ketdok",
                        "placeholder" => "tambahkan ketdok",
                        "value" => $form_data->ketdok,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "ketadm",
                        "type" => "text",
                        "fc" => "ketadm",
                        "placeholder" => "tambahkan ketadm",
                        "value" => $form_data->ketadm,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "ip_created",
                        "type" => "text",
                        "fc" => "ip_created",
                        "placeholder" => "tambahkan ip_created",
                        "value" => $form_data->ip_created,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "ip_modified",
                        "type" => "text",
                        "fc" => "ip_modified",
                        "placeholder" => "tambahkan ip_modified",
                        "value" => $form_data->ip_modified,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "perkiraan_manual",
                        "type" => "text",
                        "fc" => "perkiraan_manual",
                        "placeholder" => "tambahkan perkiraan_manual",
                        "value" => $form_data->perkiraan_manual,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "keterangan_perkiraan_manual",
                        "type" => "text",
                        "fc" => "keterangan_perkiraan_manual",
                        "placeholder" => "tambahkan keterangan_perkiraan_manual",
                        "value" => $form_data->keterangan_perkiraan_manual,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "tgl_pk",
                        "type" => "text",
                        "fc" => "tgl_pk",
                        "placeholder" => "tambahkan tgl_pk",
                        "value" => $form_data->tgl_pk,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "status_pk",
                        "type" => "text",
                        "fc" => "status_pk",
                        "placeholder" => "tambahkan status_pk",
                        "value" => $form_data->status_pk,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "statuspendidikan",
                        "type" => "text",
                        "fc" => "statuspendidikan",
                        "placeholder" => "tambahkan statuspendidikan",
                        "value" => $form_data->statuspendidikan,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "terima_pk",
                        "type" => "text",
                        "fc" => "terima_pk",
                        "placeholder" => "tambahkan terima_pk",
                        "value" => $form_data->terima_pk,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "tglpksisko",
                        "type" => "text",
                        "fc" => "tglpksisko",
                        "placeholder" => "tambahkan tglpksisko",
                        "value" => $form_data->tglpksisko,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "tglspbgtaiwan",
                        "type" => "text",
                        "fc" => "tglspbgtaiwan",
                        "placeholder" => "tambahkan tglspbgtaiwan",
                        "value" => $form_data->tglspbgtaiwan,
                    ])
                ?>
            
                <?=
                    form::input([
                        "title" => "email",
                        "type" => "text",
                        "fc" => "email",
                        "placeholder" => "tambahkan email",
                        "value" => $form_data->email,
                    ])
                ?>
            
                        <div class="form-group">
                          <button type="submit" class="btn btn-primary">Simpan</button>
                          <a class="btn btn-default" href="<?= site_url('admin/personal'); ?>">Back</a>
                        </div>
                    </form>
                </div>
            </div>
        </div>
      </div>
    </section>
</div>

